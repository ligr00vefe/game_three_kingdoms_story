...
### L4. 능력치 두루마리 (우측 패널) — **3조각으로 분리 생성**

> **폐기**: 예전 L4는 "카드 프레임 1장"이었으나, 두루마리가 펴지는 애니메이션으로 바뀌면서 폐기됐다.
> 한 장으로 만들면 펴질 때 축까지 같이 늘어나 뭉개진다.

펴지는 높이가 캐릭터마다(설명·스킬 글 길이) 다르고 애니메이션 중에 계속 변하므로
**늘어나는 조각(본문)과 안 늘어나는 조각(축)을 반드시 분리한다.**

| 조각 | 파일 | 늘어남 | 필수 조건 |
|---|---|---|---|
| L4-A 축 | `ui/scroll_rod.png` | ✗ 고정 | 위/아래 같은 아트를 상하 반전 재사용 |
| L4-B 본문 | `ui/scroll_body.png` | ○ 세로 | **세로 seamless 필수** (`repeat-y`) |
| L4-C 명패 | `ui/wood_plaque.png` | ○ 가로 | 중앙 평탄 (9-slice) |

#### L4-A. 두루마리 축 (봉)
- **크기**: 1024×128 생성 → 224×28 / **투명 PNG** (가로:세로 8:1)
- 적용: `.scroll-rod` 배경. 아트 적용 시 `.scroll-rod`의 `height`를 28px로 올릴 것
- 가로 반복 안 함 → seamless 불필요. 양끝 마구리가 아트에 포함돼야 한다
```
[STYLE PREFIX]
A single horizontal wooden scroll rod (the wooden dowel at the end of an ancient Chinese
hanging scroll), transparent PNG, strict straight-on side view, flat 2D.
Warm brown lacquered wood with subtle grain, slightly darker rounded end caps at both
far left and far right, a thin gold band ring just inside each cap, soft highlight along
the top edge and soft shadow along the bottom edge so it reads as a cylinder.
Perfectly horizontal and centered, spanning the full width of the canvas edge to edge.
Rod only — no paper attached, no hanging string, no text.
```

#### L4-B. 두루마리 본문 양피지 (세로 반복)
- **크기**: 672×672 생성 → 폭 224 / **불투명 PNG**
- 적용: `.scroll-content` background, `repeat-y`
- **가장 중요**: 위/아래 끝이 이음새 없이 맞물려야 한다. 세로로 밝기 변화(비네팅·그림자·접힘)가
  있으면 반복할 때 종이 한가운데에 그 무늬가 줄줄이 나타난다 — 성 밖 연못에서 겪은 바로 그 문제다
```
[STYLE PREFIX]
A seamless VERTICALLY tileable aged parchment paper texture for a scroll body,
flat 2D, strict straight-on view, opaque (no transparency).
Warm ivory-to-sand tone. Shading varies ONLY across the width: slightly darker along the
LEFT and RIGHT edges as if the paper curls away, with a soft even highlight down the middle.
From TOP to BOTTOM it must be perfectly uniform and flat — no vignette, no gradient,
no shadow, no folds, no creases, no torn or burnt edges, no ornament, no text.
Subtle fine fiber grain and very faint age mottling only.
The TOP and BOTTOM edges must connect seamlessly so it repeats vertically forever.
```

#### L4-C. 게임 시작 나무 명패 (선택 — 없으면 현재 CSS 유지)
- **크기**: 672×192 생성 → 224×64 / **투명 PNG** (9-slice)
- 적용: `.lobby-start` 배경
```
[STYLE PREFIX]
A horizontal wooden signboard plaque for a UI button, transparent PNG,
flat 2D straight-on view. Dark walnut plank with softly rounded corners,
thin carved bevel border, small iron rivets at the four corners,
gentle highlight along the top edge.
Center area must stay plain and evenly lit so it can be stretched (9-slice)
and so button text stays readable. No text, no icons.
```

> **생성 후 검사(L4-B)**: 같은 이미지를 세로로 2장 이어 붙여 이음새가 보이는지 먼저 확인한다.
> 이 검사를 통과하지 못하면 두루마리가 길어질수록 반복 무늬가 눈에 띈다.
...
| near (근경) | 0.55~0.6 | `bg_broken_buildings`/`bg_hills` | `img_castle_wall` | 가까운 폐허 건물/언덕 |
...
| L4-A 두루마리 축 | src/assets/scroll_rod.png | `.scroll-rod` 배경 (위/아래 반전 재사용) |
| L4-B 두루마리 본문 | src/assets/scroll_body.png | `.scroll-content` 배경, `repeat-y` |
| L4-C 나무 명패 | src/assets/wood_plaque.png | `.lobby-start` 9-slice |
